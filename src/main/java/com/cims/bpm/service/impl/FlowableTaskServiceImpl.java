...
import com.cims.bpm.dao.IFlowableTaskDao;
...
public class FlowableTaskServiceImpl extends BaseProcessService implements IFlowableTaskService {
        ...
        if (taskEntity != null) {
            ...
            if (distActivity != null) {
                if (FlowConstant.FLOW_SUBMITTER.equals(distActivity.getName())) {
                    ...
                    runtimeService.setVariable(backTaskVo.getProcessInstanceId(), "skip", "no");
                }
            }
            //5.删除节点
                    ...
                    if (StringUtils.isNotBlank(parentTaskId)) {
                        ...
                        if (subTaskCount == 0) {
                            ...
                            if (FlowConstant.AFTER_ADDSIGN.equals(task.getScopeType())) {
                                taskService.complete(parentTaskId,params.getVariables());
                            }
                        }
                    }
                }
    ...
    public PagerModel<TaskVo> getApplyedTasks(TaskQueryVo params, Query query) {
        ...
        Page<TaskVo> applyedTasks = flowableTaskDao.getApplyedTasks(params);
            for(TaskVo taskVo:applyedTasks){
                List<Comment> comments=taskService.getTaskComments(taskVo.getTaskId(),CommentTypeEnum.SP.toString());
                if(comments.size()>0){
                    taskVo.setComment(comments.get(0).getFullMessage());
                }
            }
